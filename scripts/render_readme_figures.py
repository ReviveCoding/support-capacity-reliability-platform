...
def find_one(root: Path, name: str) -> Path:
    ...
    if len(matches) != 1:
        ...
        raise RuntimeError(f"Expected exactly one {name} below {root}; found {len(matches)} ({rendered})")
    return matches[0]
...
def rect(x: float, y: float, w: float, h: float, *, fill: str, stroke: str = "none", radius: float = 10, opacity: float | None = None) -> str:
...
def line(x1: float, y1: float, x2: float, y2: float, *, stroke: str, width: float = 2, dash: str | None = None, opacity: float | None = None) -> str:
...
def polyline(points: list[tuple[float, float]], *, stroke: str, width: float = 2.5, fill: str = "none", opacity: float | None = None) -> str:
...
def text_lines(x: float, y: float, content: str, cls: str, line_height: float = 15, anchor: str = "start") -> str:
    ...
    return f'<text x="{x:.2f}" y="{y:.2f}" class="{cls}" text-anchor="{anchor}">' + "".join(tspan) + "</text>"
...
def render_pipeline(path: Path) -> None:
    ...
    body = [
        ...
        text(60, 79, "Forecast candidates are not promoted on accuracy alone: each path is evaluated through feasibility, queue performance, bounded recourse, and a fail-closed release gate.", "subtitle"),
    ]
    start_x = 45
    y = 160
    box_w = 172
    ...
    for index, (name, sub, color) in enumerate(stages):
        ...
        body.append(text_lines(x + box_w / 2, y + 86, sub.replace(" Â· ", "\n"), "box-sub", 16, "middle"))
        if index < len(stages) - 1:
            arrow_x1 = x + box_w + 4
            arrow_x2 = x + box_w + gap - 4
            arrow_y = y + box_h / 2
            body.append(line(arrow_x1, arrow_y, arrow_x2, arrow_y, stroke=COLORS["muted"], width=2.2))
            body.append(f'<polygon points="{arrow_x2:.2f},{arrow_y:.2f} {arrow_x2-9:.2f},{arrow_y-5:.2f} {arrow_x2-9:.2f},{arrow_y+5:.2f}" fill="{COLORS["muted"]}"/>')
    body.extend([
        rect(155, 368, 890, 95, fill="#f5f7fb", stroke="#cbd5e1", radius=14),
        text(205, 402, "Persisted evidence", "box-title"),
        text(205, 427, "checksummed artifact index Â· selected forecast bundle Â· monitoring snapshot Â· FastAPI surface Â· reproducible command ledger", "box-sub"),
        text(205, 448, "Offline synthetic operational replay only: no live customer traffic or causal real-time rollout claim.", "small"),
    ])
    write_svg(path, "".join(body), "Reliability-aware workforce decision pipeline", "Architecture of the forecast-to-workforce-decision platform.")
    ...
    if suffix:
        for name in names:
            if name.lower().endswith(suffix.lower()):
                return name
    raise RuntimeError(f"Could not find expected column. Candidates={candidates}, suffix={suffix}, available={names}")


def aggregate_forecast(rows: list[dict[str, str]]) -> tuple[list[str], list[float], list[float], list[float], list[float], list[bool]]:
    if not rows:
    ...
    actual_col = pick_column(fields, ["offered_load_estimate", "actual", "y_true", "target", "observed"], None)
    regime_col = next((field for field in fields if field.lower() == "regime"), None)

    buckets: dict[str, dict[str, Any]] = {}
    for row in rows:
        key = row[time_col]
        bucket = buckets.setdefault(key, {"actual": 0.0, "q10": 0.0, "q50": 0.0, "q90": 0.0, "incident": False})
        ...
        bucket["incident"] = bucket["incident"] or (str(row.get(regime_col, "")).lower() == "incident" if regime_col else False)
...
def metric_from_summary(summary: dict[str, Any], names: list[str]) -> Any:
    ...
    for key in ("fixed_origin_forecast_metrics", "selected_forecast_metrics", "forecast_tail_metrics", "selected_decision_metrics"):
...
def render_forecast(path: Path, rows: list[dict[str, str]], summary: dict[str, Any]) -> None:
    ...
    def px(index: int) -> float:
        return chart_x if len(timestamps) <= 1 else chart_x + chart_w * index / (len(timestamps) - 1)
    ...
    body = [
        ...
        text(60, 77, "Aggregate offered-load demand across region and skill. The interval band is q10â€“q90; the center line is q50.", "subtitle"),
    ...
    band_points = [(px(index), py(value)) for index, value in enumerate(q90)] + [(px(index), py(value)) for index, value in reversed(list(enumerate(q10)))]
    body.append(polygon(band_points, fill=COLORS["cyan"], opacity=0.38))
    body.append(polyline([(px(index), py(value)) for index, value in enumerate(actual)], stroke=COLORS["black"], width=2.6))
    body.append(polyline([(px(index), py(value)) for index, value in enumerate(q50)], stroke=COLORS["blue"], width=3.0))
    for index in range(0, len(timestamps), max(1, len(timestamps) // 6)):
        x = px(index)
        body.append(line(x, chart_y + chart_h, x, chart_y + chart_h + 5, stroke=COLORS["muted"], width=1))
    ...
    body.extend([
        line(legend_x, legend_y, legend_x + 26, legend_y, stroke=COLORS["black"], width=2.6), text(legend_x + 34, legend_y + 4, "Realized offered load", "axis"),
        line(legend_x + 210, legend_y, legend_x + 236, legend_y, stroke=COLORS["blue"], width=3), text(legend_x + 244, legend_y + 4, "q50 forecast", "axis"),
        rect(legend_x + 375, legend_y - 8, 26, 13, fill=COLORS["cyan"], opacity=0.38, radius=2), text(legend_x + 410, legend_y + 4, "q10â€“q90 interval", "axis"),
    ])
    side_x = 965
    ...
    metrics = [
        ("Fixed-origin WAPE", fmt_percent(metric_from_summary(summary, ["wape", "fixed_origin_wape"]))),
        ("80% interval coverage", fmt_percent(metric_from_summary(summary, ["interval_coverage_80", "fixed_origin_interval_coverage_80"]))),
        ("Peak q90 coverage", fmt_percent(metric_from_summary(summary, ["peak_q90_coverage"]))),
        ("Incident q90 coverage", fmt_percent(metric_from_summary(summary, ["incident_q90_coverage"]))),
    ]
    ...
    body.append(text(80, 492, "Shaded intervals indicate any leaf-level incident regime. Figure is generated from the exact hosted smoke evidence artifact.", "small"))
    write_svg(path, "".join(body), "Fixed-origin workload forecast with calibrated uncertainty", "Forecast uncertainty figure generated from fixed-origin prediction artifact.")
...
def render_recourse(path: Path, actions: list[dict[str, str]], summary: dict[str, Any]) -> None:
    ...
    body = [
        ...
        text(60, 77, "The release outcome is not a raw forecast score: recovery must remain inside feasibility, hard-violation, action-rate, and cost-share gates.", "subtitle"),
    ]
    chart_x, chart_y, chart_w, chart_h = 82, 152, 640, 250
    body.append(rect(chart_x, chart_y, chart_w, chart_h, fill=COLORS["panel"], stroke="#d7dee8", radius=10))
    gap = 30
    ...
    for index, (name, count) in enumerate(items):
        ...
        body.append(rect(x, y, bar_w, height, fill=[COLORS["green"], COLORS["blue"], COLORS["amber"], COLORS["purple"]][index % 4], radius=6))
        ...
        body.append(text(x + bar_w / 2, chart_y + chart_h + 28, f"cost {cost[name]:+.0f}", "axis", "middle"))
    score_x, score_y, score_w, score_h = 770, 145, 365, 300
    body.append(rect(score_x, score_y, score_w, score_h, fill="#f7f9fc", stroke="#d7dee8", radius=12))
    ...
    decision = summary.get("selected_decision_metrics", {}) if isinstance(summary.get("selected_decision_metrics"), dict) else {}
    diag = summary.get("decision_diagnostics", {}) if isinstance(summary.get("decision_diagnostics"), dict) else {}
    values = [
        ...
        ("Recourse action rate", fmt_percent(get_any(decision, ["recourse_action_rate"], diag.get("recourse_action_rate")))),
        ("Recourse cost share", fmt_percent(get_any(decision, ["recourse_cost_share"], diag.get("recourse_cost_share")))),
        ("Actions applied", str(get_any(summary, ["intraday_recourse_actions"], sum(counts.values())))),
    ]
    for index, (name, value) in enumerate(values):
        ...
        if index < len(values) - 1:
            body.append(line(score_x + 22, y + 11, score_x + score_w - 22, y + 11, stroke="#e2e8f0", width=1))
    body.append(text(82, 484, "Action bars exclude hold_schedule rows. Costs are estimated operational-cost deltas in the persisted recourse artifact.", "small"))
    write_svg(path, "".join(body), "Bounded intraday recourse closes operational gaps", "Recourse actions and release-gate outcomes from smoke evidence.")


def render_gate_scorecard(path: Path, canonical_summary: dict[str, Any], stress_summary: dict[str, Any]) -> None:
    canonical_decision = canonical_summary.get("selected_decision_metrics", {}) if isinstance(canonical_summary.get("selected_decision_metrics"), dict) else {}
    stress_decision = stress_summary.get("selected_decision_metrics", {}) if isinstance(stress_summary.get("selected_decision_metrics"), dict) else {}
    canonical_rows = canonical_summary.get("rows", {}) if isinstance(canonical_summary.get("rows"), dict) else {}
    stress_rows = stress_summary.get("rows", {}) if isinstance(stress_summary.get("rows"), dict) else {}
    ...
    cards = [
        ("Canonical smoke", f"{canonical_agents} agents", canonical_status, COLORS["green"], canonical_decision),
        ("Insufficient-workforce stress", f"{stress_agents} agents", stress_status, COLORS["red"], stress_decision),
    ]
    body = [
        text(60, 52, "Release gate distinguishes recoverable demand from structural shortfall", "title"),
        text(60, 77, "The negative case is intended to remain ITERATE: the gate rejects infeasible workforce conditions instead of silently relaxing constraints.", "subtitle"),
    ]
    x_positions = [75, 640]
    y, w, h = 135, 485, 300
    for index, (name, agents, status, color, decision) in enumerate(cards):
        ...
        footer = "Canonical replay recovered inside gate limits." if index == 0 else "Structural shortfall retained after bounded recourse."
    ...
    body.append(text(60, 486, "The stress configuration is a negative-control test, not a failed release candidate. It verifies that the release gate remains fail-closed.", "small"))
    write_svg(path, "".join(body), "Release-gate comparison between canonical and stress evidence", "Canonical PASS_WITH_RECOURSE compared with insufficient-workforce ITERATE negative gate.")
...
def main() -> None:
    ...
    reproducibility = smoke_summary.get("reproducibility", {}) if isinstance(smoke_summary.get("reproducibility"), dict) else {}
    ...
    render_forecast(args.output_dir / "fixed_origin_forecast.svg", load_csv(prediction_path), smoke_summary)
    render_recourse(args.output_dir / "bounded_recourse.svg", load_csv(action_path), smoke_summary)
    render_gate_scorecard(args.output_dir / "release_gate_comparison.svg", smoke_summary, stress_summary)
    ...
    (args.output_dir / "readme_figure_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8", newline="\n")
    print(json.dumps({"status": "PASS", "output_dir": str(args.output_dir), "figure_count": 4, "artifact_git_sha": artifact_sha or None}, indent=2))
